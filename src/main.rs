...
async fn display_main_commands(context: &Context, sender_id: i64) {
...
async fn rank(context: Context, message: Message) -> CommandResult {
    ...
    display_main_commands(&context, sender_id).await;
...
async fn contest(context: Context, message: Message) -> CommandResult {
    ...
    let sender_id = message.from.clone().unwrap().id;
    let channels = get_channels(&context, sender_id);
...
async fn start(context: Context, message: Message) -> CommandResult {
    ...
    if let Some(encoded_params) = split.next() {
        ...
        let registered = try_register(&context, chat_id, sender_id).await;
        if !registered {
            context
                .api
                .send_message(SendMessage::new(
...
async fn try_register(context: &Context, chat_id: i64, registered_by: i64) -> bool {
    ...
    let (mut invite_link, username, title, is_channel) = {
        match context.api.get_chat(GetChat { chat_id }).await.unwrap() {
            Chat::Channel(c) => {
...
fn get_channels(context: &Context, user_id: i64) -> Vec<Channel> {
    let guard = context.data.read();
    let map = guard.get::<DBKey>().expect("db");
    let conn = map.get().unwrap();
    let mut stmt = conn
...
fn get_owners(context: &Context) -> Vec<User> {
    let guard = context.data.read();
    let map = guard.get::<DBKey>().expect("db");
    let conn = map.get().unwrap();
    let mut stmt = conn
        .prepare("SELECT id, first_name, last_name, username FROM users ORDER BY id")
        .unwrap();
...
async fn delete_parent_message(context: &Context, chat_id: i64, parent_message: Option<i64>) {
...
async fn display_manage_menu(context: &Context, chat_id: i64, chan: &Channel) {
    let mut reply = SendMessage::new(
        chat_id,
...
async fn callback_handler(context: Context, update: Update) {
    ...
    if main {
        delete_parent_message(&context, chat_id, parent_message).await;
        display_main_commands(&context, sender_id).await;
        return;
    }
    ...
    if manage {
        remove_loading_icon(&context, &callback.id, None).await;
        display_manage_menu(&context, chat_id, &chan).await;
        delete_parent_message(&context, chat_id, parent_message).await;
    }
...
impl std::fmt::Display for ContestError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ContestError::ParseError(error) => write!(f, "DateTime parse {}", error),
...
async fn message_handler(context: Context, update: Update) {
    ...
    if registration_flow {
        ...
        try_register(&context, chat_id, registered_by).await;
        display_main_commands(&context, sender_id).await;
    } else {
        ...
        if text.starts_with('/') {
            ...
            if text.starts_with(&format!("/start@{}", bot_name)) && is_owner {
                let res = start(context, message.clone()).await;
                if res.is_err() {
                ...
                for command in commands {
                    if text.starts_with(&format!("/{}@{}", command, bot_name)) {
                        ...
                        let res = context
                            .api
                            .send_message(SendMessage::new(chat_id, &text))
                            .await;
        ...
        if text.split('\n').skip_while(|r| r.is_empty()).count() == 3 {
            let channels = get_channels(&context, sender_id); // channels registered by the user
            let chan = {
                let guard = context.data.read();
                let map = guard.get::<DBKey>().expect("db");
                let conn = map.get().unwrap();
            ...
            if chan.is_some() {
                ...
                display_manage_menu(&context, sender_id, &chan).await;
